...
export class Player {
  ...
  loadModel() {
    ...
    loader.load("coba1.fbx", (fbx) => {
      ...
      this.mesh.scale.set(4.5, 4.5, 4.5);
...
export class PlayerController {
  ...
  onMouseWheel(event) {
    ...
    // // Implementasi perubahan focal length
    // var focalLength = this.ThirdPersonCamera.camera.fov; // Panjang fokus awal
    // var targetFocalLength = 20; // Panjang fokus target
    // var focalLengthStep = 0.5; // Langkah ukuran untuk setiap frame

    // if (focalLength < targetFocalLength) {
    //   this.ThirdPersonCamera.camera.fov += focalLengthStep;
    // } else {
    ...
    const delta = event.deltaY * 0.05;
    const newFocalLength = this.ThirdPersonCamera.camera.getFocalLength() + (event.deltaY < 0 ? 5 : -5);
    this.ThirdPersonCamera.camera.setFocalLength(Math.max(10, Math.min(75, newFocalLength)));
